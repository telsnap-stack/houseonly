#!/usr/bin/env node
// Deep Jungle (source:dj) → Discogs review-queue backfill.
// Adapted from backfill-tv-auto.mjs (same flow, tag swapped): re-sends every
// source:dj product to the products/create webhook so the Discogs matcher
// queues them for review. Run AFTER deploying the worker with source:dj in
// ACCEPTED_SOURCE_TAGS — before that the webhook answers
...
async function fetchAllSourceProducts(token) {
...
(async () => {
  console.log(`\n=== Deep Jungle (${SOURCE_TAG}) → Discogs review queue backfill ===`);
  console.log(SEND ? 'MODE: SEND\n' : 'MODE: DRY RUN (pass --send to replay)\n');
  const token = await mintToken();
  console.log('Shopify token minted. Fetching source:dj products...');
  const products = await fetchAllSourceProducts(token);
  ...
  console.log('\n══════════════════════════════════════════════════');
  console.log(`  DEEP JUNGLE BACKFILL COMPLETE`);
  console.log(`  source:dj products with a SKU : ${sendable.length}`);
