...
async function fetchShopifyProducts() {
    ...
    products(first: 50) {
      edges {
        node {
          ...
          variants(first: 1) { edges { node { id price { amount currencyCode } quantityAvailable } } }
  ...
  return data.products.edges.map(({ node }) => {
    ...
    return {
      id: node.id, shopifyVariantId: node.variants.edges[0]?.node?.id,
      title: node.title, artist: getMeta('artist'), label: getMeta('label'),
      catalog: getMeta('catalog'), genre: getMeta('genre') || 'Deep House',
      ...
      coverUrl: image?.url || null, audio: getMeta('audio_url') || null,
      tracks: [], desc: node.descriptionHtml?.replace(/<[^>]+>/g,'') || '',
...
async function shopifyCheckout(cartItems) {
  const lines = cartItems.map(i => ({ merchandiseId: i.shopifyVariantId, quantity: i.qty })).filter(l => l.merchandiseId);
  if (!lines.length) { alert('Add real products in Shopify admin first.'); return; }
  const data = await shopifyQuery(`
...
async function claudeJSON(systemPrompt, userMsg, useSearch=false) {
  const body = { model:"claude-sonnet-4-20250514", max_tokens:3000, system:systemPrompt, messages:[{role:"user",content:userMsg}] };
...
function BulkImporter({ onImportMany }) {
  ...
  const search = async () => {
    ...
    setStatus('loading'); setResults([]); setSelected({}); setErrMsg(''); setAddedCount(0); setEnriching(false);
    try {
...
[${RECORD_SCHEMA}]
For coverUrl: extract real publicly accessible image URLs (NOT from i.discogs.com as those are blocked).
Try beatport.com or juno.co.uk for cover images.
For audioUrl: any .mp3/.ogg preview URL. Empty string if none.
genre: one of ${GENRES}. price: 18.99 default.`,
        `Find real vinyl releases for: "${query}". Return ONE JSON array only.`,
        true
      );
      const arr = Array.isArray(list)?list:[list];
      if (!arr.length) throw new Error('No releases found.');
      setResults(arr);
      const sel={};arr.forEach((_,i)=>sel[i]=true);setSelected(sel);
      setStatus('done');
    } catch(e) { setErrMsg(e.message||'Unknown error'); setStatus('error'); }
  };

  const toggleAll=()=>{ const allOn=results.every((_,i)=>selected[i]); const next={}; if(!allOn) results.forEach((_,i)=>next[i]=true); setSelected(next); };

  const add=()=>{
    const toAdd=results.filter((_,i)=>selected[i]).map(r=>({id:Date.now()+Math.random(),...r,audio:r.audioUrl||null,month:new Date().getMonth()+1,stock:10,g:'135deg,#1a1a2e,#16213e'}));
    onImportMany(toAdd); setAddedCount(toAdd.length); setResults([]); setSelected({}); setQuery(''); setStatus('idle');
  };
  ...
  return (
    <div>
      <p style={{fontSize:10,color:S.muted,margin:'0 0 12px'}}>Search by label or artist. The AI finds real releases from Discogs — pick which to add.</p>
      ...
      {status==='idle'&&!addedCount&&<div style={{display:'flex',flexWrap:'wrap',gap:5}}>{SUGGESTIONS.map(s=><button key={s} onClick={()=>setQuery(s)} style={{background:S.border,color:S.muted,border:'none',borderRadius:20,cursor:'pointer',fontSize:9,letterSpacing:1,padding:'3px 10px',textTransform:'uppercase'}}>{s}</button>)}</div>}
      {status==='loading'&&<div style={{marginTop:12,padding:14,background:S.bg,borderRadius:2,border:`1px solid ${S.border}`}}><div style={{fontSize:10,color:S.muted,marginBottom:8}}>Searching for "{query}"…</div><div style={{height:2,background:S.border,borderRadius:1,overflow:'hidden'}}><div style={{height:'100%',background:S.accent,animation:'kf 1.2s ease-in-out infinite'}} /></div><style>{`@keyframes kf{0%,100%{opacity:.4;width:20%}50%{opacity:1;width:85%}}`}</style></div>}
      {status==='error'&&<div style={{marginTop:10,padding:12,background:'#1a0000',border:`1px solid ${S.danger}33`,borderRadius:2}}><div style={{fontSize:10,color:S.danger,fontWeight:700,marginBottom:4}}>Search failed</div><div style={{fontSize:10,color:'#ff8080'}}>{errMsg}</div></div>}
      {addedCount>0&&status==='idle'&&<div style={{marginTop:10,fontSize:11,color:S.accent}}>✓ {addedCount} record{addedCount!==1?'s':''} added.</div>}
      {status==='done'&&results.length>0&&(
        <div style={{marginTop:14}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10,flexWrap:'wrap',gap:8}}>
            <span style={{fontSize:10,color:S.muted}}>Found <b style={{color:S.text}}>{results.length}</b> · <b style={{color:S.accent}}>{selCount}</b> selected {enriching&&<span style={{color:S.muted}}>⟳ fetching art…</span>}</span>
            <div style={{display:'flex',gap:6}}>
              <button onClick={toggleAll} style={{background:'none',border:`1px solid ${S.border}`,color:S.muted,cursor:'pointer',fontSize:9,letterSpacing:1.5,textTransform:'uppercase',padding:'4px 10px',borderRadius:2}}>{results.every((_,i)=>selected[i])?'Deselect All':'Select All'}</button>
...
function UrlImporter({ onImport }) {
  ...
  const run=async()=>{
    ...
    try { const r=await claudeJSON(`Extract vinyl record data from a URL. Return ONLY a JSON object: ${RECORD_SCHEMA}. genre: ${GENRES}. audioUrl: look for .mp3/.ogg preview URLs.`,`Extract record data from: ${url}`,true); setResult(r); }
  ...
  return (
    <div>
      <div style={{display:'flex',gap:8}}>
        <input value={url} onChange={e=>setUrl(e.target.value)} onKeyDown={e=>e.key==='Enter'&&run()} placeholder="Paste product URL from wordandsound.net…" style={{flex:1,background:S.bg,border:`1px solid ${S.border}`,color:S.text,borderRadius:2,padding:'8px 12px',fontSize:11,fontFamily:'inherit',outline:'none'}} />
        <Btn ch={loading?'…':'Import'} onClick={run} disabled={loading||!url.trim()} />
      </div>
      {err&&<div style={{fontSize:10,color:S.danger,marginTop:8}}>{err}</div>}
      {result&&<div style={{marginTop:12,padding:14,background:S.bg,borderRadius:2}}><div style={{fontSize:13,fontWeight:700,color:S.text}}>{result.title} — {result.artist}</div><div style={{fontSize:10,color:S.muted,marginTop:3}}>{result.label} · {result.catalog} · {result.year}</div>{result.coverUrl&&<img src={result.coverUrl} alt="" style={{width:60,height:60,objectFit:'cover',borderRadius:2,marginTop:8}} />}<div style={{marginTop:10,display:'flex',gap:8}}><Btn ch="Add to Store" onClick={confirm} /><Btn ch="Discard" variant="ghost" onClick={()=>setResult(null)} /></div></div>}
    </div>
  );
}
...
function BarcodeImporter({ onImport }) {
  ...
  const videoRef=useRef(null); const streamRef=useRef(null); const activeRef=useRef(false);
  const lookup=async(code)=>{
    setLoading(true); setErr(''); setResult(null);
    try { const r=await claudeJSON(`Find vinyl record by barcode/EAN/UPC. Return ONLY JSON: ${RECORD_SCHEMA}. genre: ${GENRES}.`,`Find vinyl record with barcode: ${code}`,true); setResult({...r,barcode:code}); }
    catch(e){setErr(e.message);}
    setLoading(false);
  };
  const startCamera=async()=>{
    ...
    try {
      ...
      setTimeout(async()=>{
        ...
        const scan=async()=>{ if (!activeRef.current) return; try { const codes=await det.detect(videoRef.current); if (codes.length){stopCamera();setBarcode(codes[0].rawValue);lookup(codes[0].rawValue);return;} } catch {} requestAnimationFrame(scan); };
        requestAnimationFrame(scan);
      },300);
    } catch {setCamErr('Could not access camera. Check permissions.');}
  };
  const stopCamera=()=>{ activeRef.current=false; setCameraOn(false); if (streamRef.current){streamRef.current.getTracks().forEach(t=>t.stop());streamRef.current=null;} };
  useEffect(()=>()=>stopCamera(),[]);
  const confirm=()=>{ onImport({id:Date.now(),...result,audio:result.audioUrl||null,month:new Date().getMonth()+1,stock:10,g:'135deg,#1a1a2e,#16213e'}); setResult(null); setBarcode(''); };
  return (
    <div>
      ...
      {cameraOn&&<div style={{marginTop:10,position:'relative',borderRadius:3,overflow:'hidden',border:`1px solid ${S.border}`}}><video ref={videoRef} style={{width:'100%',maxHeight:220,objectFit:'cover',display:'block'}} muted playsInline /><div style={{position:'absolute',inset:0,pointerEvents:'none'}}><div style={{position:'absolute',left:'10%',right:'10%',top:'50%',height:2,background:`${S.accent}88`,boxShadow:`0 0 8px ${S.accent}`,animation:'scan 1.6s ease-in-out infinite'}} /></div><div style={{position:'absolute',bottom:0,left:0,right:0,padding:'8px 12px',background:'rgba(0,0,0,0.7)',display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:9,color:S.muted,letterSpacing:1.5,textTransform:'uppercase'}}>Scanning…</span><button onClick={stopCamera} style={{background:'none',border:`1px solid ${S.border}`,color:S.muted,cursor:'pointer',fontSize:9,letterSpacing:1.5,textTransform:'uppercase',padding:'3px 8px',borderRadius:2}}>Cancel</button></div><style>{`@keyframes scan{0%,100%{top:25%}50%{top:70%}}`}</style></div>}
      ...
      {err&&<div style={{fontSize:10,color:S.danger,marginTop:8}}>{err}</div>}
      {loading&&<div style={{fontSize:10,color:S.muted,marginTop:8}}>Looking up barcode…</div>}
      {result&&<div style={{marginTop:12,padding:14,background:S.bg,borderRadius:2}}><div style={{fontSize:13,fontWeight:700,color:S.text}}>{result.title} — {result.artist}</div><div style={{fontSize:10,color:S.muted,marginTop:3}}>{result.label} · {result.catalog} · {result.year}</div>{result.coverUrl&&<img src={result.coverUrl} alt="" style={{width:60,height:60,objectFit:'cover',borderRadius:2,marginTop:8}} onError={e=>e.target.style.display='none'} />}<div style={{marginTop:10,display:'flex',gap:8}}><Btn ch="Add to Store" onClick={confirm} /><Btn ch="Discard" variant="ghost" onClick={()=>setResult(null)} /></div></div>}
    </div>
  );
}
        ...
        <div style={{padding:'0 24px 24px'}}>
          <div style={{display:'flex',gap:14,marginBottom:20,alignItems:'flex-start'}}>
            ...
            <div style={{flex:1}}>
              ...
              <input value={f.coverUrl||''} onChange={e=>set('coverUrl',e.target.value)} placeholder="https://… paste any public image URL" style={{width:'100%',background:S.bg,border:`1px solid ${S.border}`,color:S.text,borderRadius:2,padding:'8px 10px',fontSize:11,fontFamily:'inherit',outline:'none',boxSizing:'border-box'}} />
            </div>
          </div>
          ...
          <div style={{marginBottom:20}}>
            ...
            <input value={f.audio||''} onChange={e=>set('audio',e.target.value)} placeholder="https://… direct link to audio preview" style={{width:'100%',background:S.bg,border:`1px solid ${S.border}`,color:S.text,borderRadius:2,padding:'8px 10px',fontSize:11,fontFamily:'inherit',outline:'none',boxSizing:'border-box'}} />
...
function AudioBatchUploader({ records, onMatch }) {
  ...
  const handleFiles=(files)=>{
    const matched=[],unmat=[],updates={};
    Array.from(files).forEach(file=>{
      if (!/\.(mp3|ogg|wav|flac|aac)$/i.test(file.name)) return;
      const base=file.name.replace(/\.[^.]+$/,'').trim().toLowerCase();
      const record=records.find(r=>r.catalog?.toLowerCase().replace(/\s/g,'')=== base.replace(/\s/g,''));
      const url=URL.createObjectURL(file);
      if (record){updates[record.id]=url;matched.push({file:file.name,title:record.title,catalog:record.catalog});}
      else unmat.push(file.name);
    });
    setMatches(matched); setUnmatched(unmat); onMatch(updates);
  };
  return (
    <div style={{marginBottom:28}}>
      <div style={{fontSize:9,color:S.muted,letterSpacing:2,textTransform:'uppercase',marginBottom:10}}>Batch Audio Upload</div>
      <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);handleFiles(e.dataTransfer.files);}} onClick={()=>inputRef.current.click()} style={{border:`2px dashed ${drag?S.accent:S.border}`,borderRadius:3,padding:'28px 20px',textAlign:'center',cursor:'pointer',transition:'border 0.15s',background:drag?'#141400':'transparent'}}>
        <div style={{fontSize:24,marginBottom:8}}>🎵</div>
        <div style={{fontSize:11,color:drag?S.accent:S.muted,fontWeight:700,letterSpacing:1,textTransform:'uppercase'}}>{drag?'Drop to upload':'Drag audio files here or click to browse'}</div>
        <div style={{fontSize:9,color:S.muted,marginTop:6}}>Name files after catalog numbers — e.g. <span style={{color:S.text,fontFamily:'monospace'}}>QTN001.mp3</span></div>
      ...
      {matches.length>0&&<div style={{marginTop:12}}><div style={{fontSize:9,color:S.accent,letterSpacing:1,textTransform:'uppercase',marginBottom:6}}>✓ {matches.length} matched</div>{matches.map((m,i)=><div key={i} style={{display:'flex',gap:8,fontSize:10,color:S.muted,marginBottom:3}}><span style={{color:S.accent}}>▶</span><span style={{fontFamily:'monospace',color:S.text}}>{m.catalog}</span><span>→</span><span>{m.title}</span></div>)}</div>}
      {unmatched.length>0&&<div style={{marginTop:10}}><div style={{fontSize:9,color:'#ff8800',letterSpacing:1,textTransform:'uppercase',marginBottom:6}}>⚠ {unmatched.length} unmatched</div>{unmatched.map((f,i)=><div key={i} style={{fontSize:10,color:S.muted,fontFamily:'monospace',marginBottom:2}}>{f}</div>)}</div>}
    </div>
  );
}
  ...
  return (
    <div style={{maxWidth:860,margin:'0 auto',padding:'36px 20px'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:28}}>
        <div><h1 style={{margin:0,fontSize:18,fontWeight:800}}>Admin Panel</h1><div style={{fontSize:10,color:S.muted,marginTop:4}}>{records.length} records · {records.reduce((s,r)=>s+r.stock,0)} units in stock</div></div>
      ...
      <div style={{background:S.surf,border:`1px solid ${S.border}`,borderRadius:3,padding:22,marginBottom:28}}>
        ...
        <div style={{display:'flex',gap:6,marginBottom:18}}>{tabBtn('bulk','⬇ Bulk Search')}{tabBtn('barcode','▥ Barcode')}{tabBtn('url','🔗 Single URL')}</div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:1}}>
        {records.map(r=>(
          <div key={r.id} style={{display:'flex',alignItems:'center',gap:12,background:S.surf,padding:'10px 14px',borderRadius:2}}>
            ...
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:700,color:S.text,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.title}</div><div style={{fontSize:9,color:S.muted}}>{r.artist} · {r.label} · {r.catalog}</div></div>
...
export default function App() {
  ...
  useEffect(()=>{
    fetchShopifyProducts()
      .then(products=>{ if(products.length){setRecords(products);setShopifyLoaded(true);} })
    ...
    <div style={{background:S.bg,minHeight:'100vh',color:S.text,fontFamily:"'Inter',system-ui,sans-serif"}}>
      <Nav>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" style={{background:S.surf,border:`1px solid ${S.border}`,color:S.text,borderRadius:2,padding:'5px 11px',fontSize:11,fontFamily:'inherit',outline:'none',width:160}} />
          ...
          <button onClick={()=>setCartOpen(true)} style={{background:cartCount>0?S.accent:S.surf,color:cartCount>0?'#080808':S.muted,border:`1px solid ${S.border}`,borderRadius:2,padding:'5px 14px',cursor:'pointer',fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:'uppercase'}}>Cart{cartCount>0?` (${cartCount})`:''}</button>
        </div>
      </Nav>

      <div style={{padding:'72px 24px 56px',textAlign:'left',borderBottom:`1px solid ${S.border}`,maxWidth:1100,margin:'0 auto'}}>
        <Logo scale={2.2} />
        <p style={{color:S.muted,fontSize:11,margin:'20px 0 0',letterSpacing:3,textTransform:'uppercase'}}>Deep · Acid · Tech · Soul &nbsp;·&nbsp; Vinyl Delivered Worldwide</p>
      </div>
